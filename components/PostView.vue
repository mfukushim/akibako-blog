<template>
  <article v-if="show">
    <div class="d-flex flex-no-wrap  flex-column">
      ...
      <div class="d-flex flex-row">
        <div class="ma-2">
          <DateView :article="article"></DateView>
        </div>
        <div class="ma-2">
          <CategoryChipView :body="body"></CategoryChipView>
        </div>
        <v-spacer></v-spacer>
        <v-btn outlined target="_blank" style="text-transform: none" small :href="postUrl(body.parentCid)">
          <v-img width="24" height="24" :src="base('ipfs-logo-vector-ice-text.svg')"></v-img>
          {{ body.parentCid }}
        </v-btn>
      </div>
    </div>
    ...
    <div>
      <a href="https://twitter.com/share?ref_src=twsrc%5Etfw" class="twitter-share-button" data-show-count="false"><v-icon>mdi-twitter</v-icon></a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
    </div>
  </article>
</template>
